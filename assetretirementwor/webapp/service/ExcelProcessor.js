    ...
    return class ExcelProcessor {
      constructor(options = {}) {
        ...
        this._columnMapping = {
          ...
          FixedAssetRetirementType: ["FixedAssetRetirementType", "Retirement Type", "retirement_type", "Asset Retirement Type",
            "RetirementType", "Fixed Asset Retirement Type", "Retirement Type Code", "Type"],
          ...
          FxdAstRtrmtRevnTransCrcy: ["FxdAstRtrmtRevnTransCrcy", "Revenue Currency", "revenue_currency", "Currency", "Currency Code"],
          AstRtrmtAmtInTransCrcy: ["AstRtrmtAmtInTransCrcy", "Retirement Amount", "retirement_amount", "Amount"],
          FxdAstRtrmtQuantityInBaseUnit: ["FxdAstRtrmtQuantityInBaseUnit", "Quantity", "quantity", "Base Quantity"],
          BaseUnitSAPCode: ["BaseUnitSAPCode", "Base Unit SAP Code", "base_unit_sap_code", "SAP Unit", "Unit SAP Code", "Base Unit SAP Code"],
          BaseUnitISOCode: ["BaseUnitISOCode", "Base Unit ISO Code", "base_unit_iso_code", "ISO Unit", "Unit ISO Code", "Base Unit ISO Code"],
      ...
      _findColumnValue(row, fieldName) {
        ...
        // Debug available columns vs what we're looking for
        if (fieldName === "FixedAssetRetirementType") {
          console.log(`Searching for ${fieldName} using possible names:`, possibleNames);
        ...
        for (const columnName of possibleNames) {
          ...
          if (row.hasOwnProperty(columnName)) {
            if (fieldName === "FixedAssetRetirementType") {
              console.log(`Found direct match for ${fieldName}: ${columnName} = ${row[columnName]}`);
            }
          ...
          for (const key of Object.keys(row)) {
            if (key.toLowerCase() === lowercaseColumnName) {
              if (fieldName === "FixedAssetRetirementType") {
                console.log(`Found case-insensitive match for ${fieldName}: ${key} = ${row[key]}`);
              }
              return row[key];
            }
          }
        ...
        if (fieldName === "FixedAssetRetirementType") {
          console.warn(`No match found for ${fieldName}`);
        }
        return undefined;
      }
      ...
      _parseFixedAssetSheet(workbook, sheetName) {
        try {
          ...
          // Debug the first row to check column names
          if (sheetData.length > 0) {
            console.log("First row column names:", Object.keys(sheetData[0]));
          ...
          const entries = sheetData.map((row, index) => {
            ...
            const entry = {
              ...
              // Fixed Asset Retirement fields using column mapping
              ...
              AstRtrmtAmtInTransCrcy: this._findColumnValue(row, "AstRtrmtAmtInTransCrcy"),
            ...
            // Log the retirement type specifically for the first few entries
            if (index < 3) {
              console.log(`Entry ${index + 1} Retirement Type:`, entry.FixedAssetRetirementType);
            }
      ...
      createTemplateFile() {
        ...
        const sampleData = [
          {
            ...
            AstRtrmtAmtInTransCrcy: "5000.00",
        ...
        window.XLSX.utils.book_append_sheet(wb, ws, "Fixed Asset Retirement");
      ...
      _addHelpSheet(wb) {
        ...
        helpData.unshift(
          ...
          },
          {
            "Field Name": "",
            Description:
              "Amounts should be entered without currency symbols (e.g., 5000.00)."
          },
          {
            "Field Name": "",
            Description: ""
