sap.ui.define([
   ...
   */
  return Control.extend("customervendor.control.BatchProcessingDisplay", {
    metadata: {
      properties: {
        ...
        transactionId: { type: "string", defaultValue: null },
    ...
    init: function () {
      ...
      const mainContent = new VBox({
        ...
        items: [
          ...
          // Transaction ID display - only visible when there is a transaction ID
          new HBox({
            justifyContent: "SpaceBetween",
            alignItems: "Center",
            visible: {
              path: "transactionId",
              formatter: function (transactionId) {
                return transactionId !== null && transactionId !== "";
              }
            },
            items: [
              new Text({ text: "Transaction ID:" }),
    ...
    // Add setter method for transaction ID
    setTransactionId: function (sValue) {
      this.setProperty("transactionId", sValue, true);
      this.getModel().setProperty("/transactionId", sValue);
      return this;
    },
    ...
    // Add transaction ID copy handler
    onCopyTransactionId: function () {
      const transactionId = this.getProperty("transactionId");
      if (transactionId) {
        try {
