  ...
  return function (oController) {
    ...
    this.processExcelFile = function (file) {
      ...
      reader.onload = (e) => {
        try {
          ...
          const requiredSheets = ["Header", "Vendor Lines", "Customer Lines"];
    ...
    this._parseMultiSheetWorkbook = function (workbook) {
      try {
        ...
        const vendorSheetName = options.vendorSheet || "Vendor Lines";
        const customerSheetName = options.customerSheet || "Customer Lines";
        ...
        if (!workbook.Sheets[vendorSheetName]) {
          throw new Error(`Vendor Lines sheet "${vendorSheetName}" not found in workbook`);
        }
        if (!workbook.Sheets[customerSheetName]) {
          throw new Error(`Customer Lines sheet "${customerSheetName}" not found in workbook`);
        }
        ...
        // Parse Vendor and Customer Sheets
        const vendorSheet = workbook.Sheets[vendorSheetName];
        const vendorData = XLSX.utils.sheet_to_json(vendorSheet, { header: 1 }).slice(1);
        const customerSheet = workbook.Sheets[customerSheetName];
        const customerData = XLSX.utils.sheet_to_json(customerSheet, { header: 1 }).slice(1);
        ...
        headerRows.forEach((headerRow) => {
          ...
          // Map vendor entries with correct column structure
          const mappedVendorEntries = vendorData
            .filter((row) => {
          ...
          // Map customer entries with correct column structure
          const mappedCustomerEntries = customerData
            .filter((row) => {
              // Normalize both sequence IDs for comparison
              const rowSequenceId = normalizeSequenceId(row[0]);
              return rowSequenceId === normalizedSequenceId;
          ...
          transactionGroups[normalizedSequenceId].entries.push(...mappedVendorEntries, ...mappedCustomerEntries);
        });
