sap.ui.define([
  "sap/ui/model/odata/v4/ODataModel", // Ensure this matches your project's OData version
  "sap/ui/core/format/DateFormat",
  "sap/ui/core/Core", // Added Core dependency for MessageManager
  "sap/ui/model/odata/v4/Context", // Added Context dependency for type check
  "sap/ui/core/message/Message" // Added Message dependency for type check
], function (ODataModel, DateFormat, Core, Context, Message) { // Added Core, Context, Message
  "use strict";

  return {
    /**
     * Factory function to create an ODataService instance.
    ...
    create: function (options) {
      ...
      const _entitySetPath = "/FixedAssetAcquisition/SAP__self.Post"; // Action Import Path
                ...
                messageInfo = {
                  message: response.message ||
                    ...
                    (response.FixedAssetPostingUUID ?
                      `Asset transaction posted with document no. ${response.FixedAssetPostingUUID}` :
      ...
      return {
        ...
        /**
         * Processes a single fixed asset acquisition record using an individual POST request ($direct).
         * Uses createCompleted event listener on the ListBinding.
         * @param {Object} assetEntry - The fixed asset entry to process.
         ...
         */
        processSingleFixedAssetAcquisition: function (assetEntry) {
          const entryId = assetEntry?._originalIndex ?? "unknown";
          _logInfo(`Processing entry index ${entryId} using $direct (createCompleted EVENT LISTENER - Final Attempt)`);
          ...
          return new Promise((resolve) => {
              ...
              if (eventContext && context && (eventContext === context || eventContext.getPath() === context.getPath())) {
                ...
                try {
                  ...
                  if (bSuccess) {
                    ...
                    _logInfo(`Record index ${entryId} processed successfully.`, response);
            ...
            try {
              // --- Transformation ---
              const transformedPayloadArray = _dataTransformer.transformFixedAssetToOData([assetEntry]);
              const oDataPayload = transformedPayloadArray && transformedPayloadArray[0];
              if (!oDataPayload) {
                throw new Error(`Data transformation returned empty result for entry ${entryId}.`);
              }
              _logInfo(`Payload for index ${entryId}:`, oDataPayload);
        ...
        }, // End of processSingleFixedAssetAcquisition
