sap.ui.define([
    ...
    "fixedassetacquisition/utils/ErrorHandler", // Assuming ErrorHandler path
    "fixedassetacquisition/utils/DataTransformer", // Assuming DataTransformer path
    "fixedassetacquisition/service/UIManager" // Assuming UIManager path
], function (JSONModel, ErrorHandler, DataTransformer, UIManager) {
    "use strict";

    /**
     * BatchProcessingManager
     * Manages the sequential processing of records, displaying progress via UIManager.
     */
    return class BatchProcessingManager {
        /**
         * Constructor for BatchProcessingManager.
         * @param {object} [options={}] - Configuration options.
         * @param {sap.ui.core.mvc.Controller} options.controller - REQUIRED.
         * @param {object} options.uiManager - An instance of the UIManager class. REQUIRED.
         * @param {ErrorHandler} [options.errorHandler] - Optional.
         * @param {DataTransformer} [options.dataTransformer] - Optional.
         * @param {object} [options.oDataService] - Optional.
         * @param {object} [options.exportManager] - Optional.
         * @param {number} [options.processingDelay=300] - Delay in ms.
         */
        constructor(options = {}) {
        ...
        initBatchProcessing(totalRecords) {
            console.log(`BatchProcessingManager: Initializing for ${totalRecords} records.`);
            ...
            const batchDisplayModel = new JSONModel({ /* ... model properties ... */
            ...
            if (!this.oController?.getView()) {
                const errorMsg = "BatchProcessingManager: Controller/View missing for model setup."; console.error(errorMsg); return Promise.reject(new Error(errorMsg));
            }
            this.oController.getView().setModel(batchDisplayModel, "batchDisplay");

            const dialogId = "batchProcessingDialog";
            const fragmentName = "fixedassetacquisition.view.BatchProcessingDisplay";

            return this._uiManager.showDialog(dialogId, fragmentName, batchDisplayModel, "batchDisplay", this.oController)
                .then(oDialog => {
                    ...
                    try { // Hide export panel initially
                        const exportPanel = sap.ui.core.Fragment.byId(this.oController.getView().createId(dialogId), "exportPanel");
                    ...
                    this._errorHandler.showError("Failed to initialize processing dialog.", err.message); throw err;
                });
        }

        /** Starts the sequential processing loop. */
        startBatchProcessing(data, processRecordFn) {
            console.log(`BatchProcessingManager: Starting processing. Data length: ${data?.length}.`);
            return new Promise((resolve, reject) => {
                if (!data?.length) { const msg = "No data."; console.warn(msg); this._errorHandler.showWarning(msg); this.closeBatchProcessingDialog(); return reject(new Error(msg)); }
                if (typeof processRecordFn !== 'function') { const msg = "Invalid processRecordFn."; console.error(msg); this._errorHandler.showError(msg); this.closeBatchProcessingDialog(); return reject(new Error(msg)); }
                ...
                this.initBatchProcessing(data.length)
                    .then(oDialog => {
                        ...
                        console.log("BatchProcessingManager: Dialog ready. Starting loop.");
                        return this._processRecordsSequentially(data, processRecordFn);
                    })
                    .then(() => { console.log("DEBUG: BatchProcMgr: Loop finished."); resolve(this.responseData); })
                    .catch(error => {
                        console.error("DEBUG: BatchProcMgr: Critical error during setup/exec:", error);
                        try { this._updateBatchDisplay({ status: "Critical Error", error: error.message, isCompleted: true, isError: true }); this._handleBatchProcessingComplete(); }
                        catch (finalError) { console.error("DEBUG: BatchProcMgr: Error during final error handling:", finalError); }
                        reject(error);
                    });
            });
        }
                ...
                const processNext = async (index) => {
                    try {
                        if (this.processingCancelled || index >= data.length) {
                            console.log(`BatchProcessingManager: Stopping loop (Cancelled: ${this.processingCancelled}, Index: ${index}/${data.length})`);
                        ...
                        this._updateBatchDisplay({ status: `Processing record ${index + 1}/${data.length}...`, currentBatch: index + 1 });
                        console.log(`DEBUG: BatchProcMgr: Calling processRecordFn idx ${index}`);

                        const result = await processRecordFn(currentRecord, index); // Await the user-provided function
                        console.log(`DEBUG: BatchProcMgr: processRecordFn resolved idx ${index}`, result);
                        ...
                        const entryId = currentRecord?._originalIndex ?? currentRecord?.SequenceID ?? `Index ${index}`;
                        console.error(`DEBUG: BatchProcMgr: *** UNEXPECTED loop error idx ${index} (ID: ${entryId}) ***:`, error);
                        this._handleRecord(false, currentRecord, null, { error: error, message: error.message || "Unexpected loop error.", errorCode: "LOOP_ERROR" }, index);
        ...
        _handleRecord(isSuccess, originalInput, responseData, errorInfo, index) {
            ...
            try {
                ...
                const fallbackMsg = this._errorHandler.createStandardMessage('error', 'INTERNAL_HANDLING_ERROR', `Internal error processing result for ${entryId}: ${handleError.message}`, { originalInput, originalIsSuccess, originalResponseData, originalErrorInfo, handleError: handleError.message, stackTrace: handleError.stack }, 'BatchProcessorInternal', entryId, index);
        ...
        _createComprehensiveMessage(isSuccess, originalInput, responseData, errorInfo) {
            ...
            try {
                ...
                console.error(`Failed to extract ${isSuccess ? 'success' : 'error'} details for entry ${entryId}:`, extractionError);
                // Fallback structure for extraction failures
                extractedInfo = {
                    code: 'EXTRACTION_FAILED',
                    message: `Failed to extract ${isSuccess ? 'success' : 'error'} details for record ${entryId}`,
            ...
            return this._errorHandler.createStandardMessage(
                ...
                messageDetails,
                'BatchProcessor', // Source
                entryId,          // Entity ID
                this.currentIndex // Batch Index
            );
        }

        // REMOVED: _extractSuccessDetails - Logic moved to ErrorHandler.extractSuccessInfo
        ...
        _handleBatchProcessingComplete() {
            ...
            this._processingComplete = true; console.log("DEBUG: BPM: Handling completion.");
            ...
            else if (t === 0) { status = "Finished: No records provided."; }
            ...
            else if (f === t && t > 0) { status = `Completed. All ${t} failed.`; isError = true; }
            else if (f > 0) { status = `Completed. ${s} succeeded, ${f} failed.`; isError = true; }
            else if (s === t && t > 0) { status = `Completed successfully (${s} records).`; }
        ...
        cancelBatchProcessing() {
            if (!this.processingCancelled && !this._processingComplete) {
                this.processingCancelled = true; console.log("BPM: Cancellation requested.");
        ...
        closeBatchProcessingDialog() {
            this._uiManager.closeDialog("batchProcessingDialog", true); // true = destroy
