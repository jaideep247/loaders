sap.ui.define(
  ["sap/m/MessageToast", "sap/m/MessageBox", "sap/ui/core/library"], // Added MessageType
  function (MessageToast, MessageBox, coreLibrary) {
    ...
    /**
     * @class ErrorHandler
     * @description Centralizes error handling, formatting, and display logic.
     */
    return class ErrorHandler {
      ...
      showError(error, title = "Error", isBusinessError = false) {
        ...
        // Show MessageBox
        this.messageBox.error(errorInfo.message || "An error occurred", {
          title: title,
      ...
      extractSuccessInfo(response) {
        ...
        try {
          if (response && typeof response === 'object') {
            ...
            // --- Add Specific Fixed Asset Logic (Example) ---
            // Override generic message/code if specific fields are present
            if (response.FixedAssetPostingUUID) {
              successInfo.code = "ASSET_POSTED";
              successInfo.message = `Asset transaction posted with document no. ${response.FixedAssetPostingUUID}`;
              ...
              successInfo.details = {
                ...
                amount: response.AcqnAmtInTransactionCurrency,
      ...
      formatValidationErrors(errors) {
        ...
        const formattedErrors = errors.map((error, index) => {
          if (typeof error === "object" && error !== null) {
            ...
            if (!message && error.toString() === "[object Object]") { message = error.field ? `Validation error in field '${error.field}'` : `Unknown validation error object (Row ${index + 1})`; }
            else if (!message) { message = `Unknown error object (Row ${index + 1}): ${JSON.stringify(error)}`; }
            return { ...error, message: String(message), sequenceId: error.sequenceId || error.SequenceNumber || error.seqId || error.entryIdentifier || "N/A", field: error.field || error.Field || error.propertyref || error.target || "Unknown" };
          } else if (typeof error === "string" && error.trim() !== "") { return { message: error, sequenceId: "N/A", field: "Unknown" }; }
          else { console.warn(`ErrorHandler: Invalid error item at index ${index}:`, error); return { message: `Invalid error item found at row ${index + 1}.`, sequenceId: "N/A", field: "Invalid Data" }; }
        }).filter(e => e && e.message);
        const uniqueErrorsMap = new Map();
        formattedErrors.forEach(error => { const key = `${error.sequenceId}-${error.field}-${error.message}`; if (!uniqueErrorsMap.has(key)) { uniqueErrorsMap.set(key, error); } });
