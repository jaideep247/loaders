sap.ui.define([
  ...
  return function (oController) {
    ...
    this.showEntryDetailsDialog = function (oEntry) {
      ...
      // Determine entry type - Bank Line or Customer Line
      const isBank = oEntry["Sheet"] === "Bank Lines";
      const isCustomer = oEntry["Sheet"] === "Customer Lines";
      ...
      // Define lineFieldsToUse before using it
      const lineFieldsToUse = isBank ? bankLineFields : customerLineFields;
      ...
      if (isCustomer) {
        // The customer line mapping issues observed from console logs
        ...
        if (itemTextValue && /^\d+$/.test(itemTextValue) &&
          ...
          console.log("Fixing misaligned customer line fields");
      ...
      if (isBank) {
        ...
        if (!correctedEntry["Reference Key 1"] && correctedEntry["Reference Key"]) {
          correctedEntry["Reference Key 1"] = correctedEntry["Reference Key"];
        }
      } 
      ...
      lineFieldsToUse.forEach(fieldName => {
        ...
        // Always add all fields, even if empty
        lineDetails.push({ key: fieldName, value: value || "" });
      ...
      const dialogTitle = isBank ? "Bank Line Entry Details" : "Customer Line Entry Details";
      const lineHeaderText = isBank ? "Bank Line Information" : "Customer Line Information";
            ...
            items: [
              ...
              new sap.m.ObjectHeader({
                ...
                attributes: isBank ? [
                  ...
                  new sap.m.ObjectAttribute({
                    text: "Customer Code: " + (getFieldValue("Customer Code", correctedEntry) || "N/A")
                  }),
                  new sap.m.ObjectAttribute({
      ...
      // Add validation errors section if present - FIXED CODE HERE
      try {
        if (correctedEntry.validationErrors) {
          // Make sure validationErrors is an array
          const errorArray = Array.isArray(correctedEntry.validationErrors)
            ? correctedEntry.validationErrors
            : [correctedEntry.validationErrors];
  ...
  */
    ...
    */
    this.showValidationStatsDialog = function (total, successful, failed) {
      // Load the validation stats dialog fragment if it doesn't exist
      if (!this._dialogs.validationStatsDialog) {
        Fragment.load({
          name: "customercollection.view.ValidationStatsDialog",
          controller: this.oController
        }).then((oDialog) => {
          // Store the dialog for future use
          this._dialogs.validationStatsDialog = oDialog;
          // Add dependency to view
          this.oController.getView().addDependent(this._dialogs.validationStatsDialog);
          // Set up the stats data
          this._updateValidationStatsDialog(total, successful, failed);
          // Open the dialog
          this._dialogs.validationStatsDialog.open();
        }).catch((error) => {
          console.error("Error loading fragment:", error);
          // Fallback to standard message box if fragment loading fails
          sap.m.MessageBox.information("Validation Statistics: " +
    ...
    /**
     * Update validation stats dialog with data
     * @param {number} total - Total entry count
     * @param {number} successful - Successful entry count
     * @param {number} failed - Failed entry count
     * @private
     */
    this._updateValidationStatsDialog = function (total, successful, failed) {
      // Create a model for the stats data
      const oModel = new JSONModel({
        total: total,
        successful: successful,
        failed: failed,
        validPercentage: Math.round((successful / total) * 100)
      });

      // Set the model on the dialog
      this._dialogs.validationStatsDialog.setModel(oModel, "validationStats");
    };
    /**
    ...
    this.showHelpDialog = function () {
      ...
      if (!this._dialogs.helpDialog) {
        Fragment.load({
          name: "customercollection.view.HelpDialog",
          controller: this.oController
        }).then((oDialog) => {
          // Store the dialog for future use
    ...
    this.showSuccessWithDocuments = function (successEntries) {
      ...
      if (!this._dialogs.successDialog) {
        Fragment.load({
          name: "customercollection.view.SuccessDialog",
          controller: this.oController
    ...
    this.showPartialSuccessWithDocuments = function (successEntries, failedEntries, successCount, failedCount) {
      ...
      if (!this._dialogs.partialSuccessDialog) {
        Fragment.load({
          name: "customercollection.view.PartialSuccessDialog",
          controller: this.oController
        }).then(function (oDialog) {
    ...
    this.showErrorWithDetails = function (failedEntries) {
      ...
      if (!this._dialogs.errorDialog) {
        Fragment.load({
          name: "customercollection.view.ErrorDialog",
          controller: this.oController
        }).then(function (oDialog) {
    ...
    this.initBatchProcessingDisplay = function (container) {
      ...
      return new Promise((resolve, reject) => {
        ...
        Fragment.load({
          name: "customercollection.view.BatchProcessingDisplay",
          controller: this.oController
        }).then(oFragment => {
