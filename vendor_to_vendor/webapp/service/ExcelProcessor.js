  ...
  return function (oController) {
    ...
    this.processExcelFile = function (file) {
      ...
      reader.onload = (e) => {
        try {
          ...
          const requiredSheets = ["Header", "Vendor Debit Lines", "Vendor Credit Lines"];
          ...
          if (!validationResult.isValid) {
            ...
            const enhancedErrors = validationResult.errors.map(error => {
              ...
              if (error.sheet) {
                ...
                else if (error.includes("Debit")) errorObj.sheet = "Vendor Debit Lines";
                else if (error.includes("Credit")) errorObj.sheet = "Vendor Credit Lines";
              }
    ...
    this._parseMultiSheetWorkbook = function (workbook) {
      try {
        ...
        const vendorDebitSheet = options.debitSheet || "Vendor Debit Lines";
        const vendorCreditSheet = options.creditSheet || "Vendor Credit Lines";
        ...
        if (!workbook.Sheets[vendorDebitSheet]) {
          throw new Error(`Vendor Debit Lines sheet "${vendorDebitSheet}" not found in workbook`);
        }
        if (!workbook.Sheets[vendorCreditSheet]) {
          throw new Error(`Vendor Credit Lines sheet "${vendorCreditSheet}" not found in workbook`);
        }
        ...
        // Parse Vendor Debit and Credit Sheets
        const debitSheet = workbook.Sheets[vendorDebitSheet];
        const debitData = XLSX.utils.sheet_to_json(debitSheet, { header: 1 }).slice(1);
        const creditSheet = workbook.Sheets[vendorCreditSheet];
        ...
        headerRows.forEach((headerRow) => {
          ...
          // Map vendor credit entries with correct column structure
