  ...
  return function (oController) {
    ...
    this.processExcelFile = function (file) {
      ...
      reader.onload = (e) => {
        try {
          ...
          const requiredSheets = ["Header", "Bank Lines", "Vendor Lines"];
          ...
          if (!validationResult.isValid) {
            ...
            const enhancedErrors = validationResult.errors.map(error => {
              ...
              if (error.sheet) {
                ...
                else if (error.includes("Debit")) errorObj.sheet = "Bank Lines";
                else if (error.includes("Credit")) errorObj.sheet = "Vendor Lines";
              }
    ...
    this._parseMultiSheetWorkbook = function (workbook) {
      try {
        ...
        const bankSheet = options.debitSheet || "Bank Lines";
        const vendorSheet = options.creditSheet || "Vendor Lines";
        ...
        if (!workbook.Sheets[bankSheet]) {
          throw new Error(`Bank line sheet "${bankSheet}" not found in workbook`);
        }
        if (!workbook.Sheets[vendorSheet]) {
          throw new Error(`Vendor line sheet"${vendorSheet}" not found in workbook`);
        }
        ...
        // Parse Debit and Credit Sheets
        const debitSheet = workbook.Sheets[bankSheet];
        const debitData = XLSX.utils.sheet_to_json(debitSheet, { header: 1 }).slice(1);
        const creditSheet = workbook.Sheets[vendorSheet];
        ...
        headerRows.forEach((headerRow) => {
          ...
          // Map debit entries with correct column structure
          const mappedDebitEntries = debitData
            ...
            .map((debitEntry) => {
              const [
                ...
                glAccount, currency, amount, assignment,
                referenceId1, itemText, houseBank,
                accountId, profitCenter, businessPlace
              ] = debitEntry;
              ...
              return {
                ...
                "Reference Key 1": referenceId1 !== undefined && referenceId1 !== null ? String(referenceId1) : referenceId1,
                "Item Text": itemText,
                ...
                "Sheet": bankSheet,
                "Entry Type": "Debit" // Add entry type for easier filtering
              };
            });

          // Map credit entries with correct column structure
          const mappedCreditEntries = creditData
            ...
            .map((creditEntry) => {
              ...
              return {
                ...
                "Amount": amount,
                "Assignment": assignment,
                "Reference Key 1": referenceKey1 !== undefined && referenceKey1 !== null ? String(referenceKey1) : referenceKey1,
                "Item Text": itemText,
                "Business Place": businessPlace,
