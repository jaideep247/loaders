sap.ui.define([
  ...
  return function (oController) {
    ...
    this.showEntryDetailsDialog = function (oEntry) {
      ...
      // Determine entry type - Customer Debit or Customer Credit
      const isCustomerDebit = oEntry["Sheet"] === "Customer Debit Lines";
      const isCustomerCredit = oEntry["Sheet"] === "Customer Credit Lines";
      ...
      // Process line fields
      customerLineFields.forEach(fieldName => {
      ...
      Object.entries(correctedEntry).forEach(([key, value]) => {
        if (!excludedKeys.includes(key) &&
          ...
          !customerLineFields.includes(key) &&
      ...
      const dialogTitle = isCustomerDebit ? "Customer Debit Line Entry Details" : "Customer Credit Line Entry Details";
      const lineHeaderText = isCustomerDebit ? "Customer Debit Line Information" : "Customer Credit Line Information";
              ...
              new sap.m.ObjectHeader({
                ...
                statuses: [
                  new sap.m.ObjectStatus({
                    text: isCustomerDebit ? "Debit" : "Credit",
                    state: isCustomerDebit
    ...
    this.showErrorSummaryDialog = function (errors, exportManager) {
      ...
      if (!this._dialogs.errorSummaryDialog) {
        Fragment.load({
          name: "customertocustomer.view.ErrorSummaryDialog",
    ...
    */
    this.showHelpDialog = function () {
      ...
      if (!this._dialogs.helpDialog) {
        Fragment.load({
          name: "customertocustomer.view.HelpDialog",
          controller: this.oController
        }).then((oDialog) => {
          // Store the dialog for future use
    ...
    this.showSuccessWithDocuments = function (successEntries) {
      ...
      if (!this._dialogs.successDialog) {
        Fragment.load({
          name: "customertocustomer.view.SuccessDialog",
          controller: this.oController
    ...
    this.showPartialSuccessWithDocuments = function (successEntries, failedEntries, successCount, failedCount) {
      ...
      if (!this._dialogs.partialSuccessDialog) {
        Fragment.load({
          name: "customertocustomer.view.PartialSuccessDialog",
          controller: this.oController
        }).then(function (oDialog) {
    ...
    this.showErrorWithDetails = function (failedEntries) {
      ...
      if (!this._dialogs.errorDialog) {
        Fragment.load({
          name: "customertocustomer.view.ErrorDialog",
          controller: this.oController
        }).then(function (oDialog) {
    ...
    this.initBatchProcessingDisplay = function (container) {
      ...
      return new Promise((resolve, reject) => {
        ...
        Fragment.load({
          name: "customertocustomer.view.BatchProcessingDisplay",
          controller: this.oController
        }).then(oFragment => {
