sap.ui.define([
  ...
  return function(oController) {
    ...
    this._createTemplateWorkbook = function() {
      ...
      // Customer Debit Lines Transactions Sheet
      const debitHeaders = [
        "Sequence ID",
      ...
      const debitSheet = XLSX.utils.aoa_to_sheet([
        debitHeaders,
        ...debitData
      ]);
      XLSX.utils.book_append_sheet(wb, debitSheet, "Customer Debit Lines");

      // Customer Credit Lines Transactions Sheet
      const creditHeaders = [
        "Sequence ID",
        "Reference Document Item",
        "Indicator (S-Dr, H-Cr.)",
      ...
      const creditSheet = XLSX.utils.aoa_to_sheet([
        creditHeaders,
        ...creditData
      ]);
     ...
     * Each journal entry requires both Customer Debit Lines and Customer Credit Lines entries
    ...
    this._validateRequiredSheets = function(sequenceGroups) {
      ...
      for (const [sequenceId, entries] of Object.entries(sequenceGroups)) {
        ...
        // Check if we have both Customer Debit Lines and Customer Credit Lines
        if (!sheets.has("Customer Debit Lines")) {
          groupErrors.push({
            message: `Sequence ID ${sequenceId}: Missing Customer Debit Lines entry`,
            sheet: "Sheet Validation",
            sequenceId: sequenceId
          });
        }
        
        if (!sheets.has("Customer Credit Lines")) {
          groupErrors.push({
            message: `Sequence ID ${sequenceId}: Missing Customer Credit Lines entry`,
            sheet: "Sheet Validation",
            sequenceId: sequenceId
          });
        }
