sap.ui.define([
  ...
  return BaseObject.extend("gigr.model.ModelManager", {
    /**
    ...
    initializeModels: function () {
      ...
      this._initgigrDocumentsModel();
      this._initBatchDisplayModel();

      // Get gigr document model from models module
    ...
    /**
     * Initialize gigr documents model
     * @private
     */
    _initgigrDocumentsModel: function () {
      const gigrDocumentsModel = new JSONModel({
        entries: [],
        validationStatus: "Pending",
        filteredCount: 0
      });
      this._view.setModel(gigrDocumentsModel, "gigrDocuments");

      // Store a reference to make it accessible to the controller if needed
      this.gigrDocumentsModel = gigrDocumentsModel;
    },
    ...
    /**
     * Reset Gigr documents model
     * @private
     */
    _resetgigrDocumentsModel: function () {
      const oModel = this._view.getModel("gigrDocuments");
      if (oModel) {
        oModel.setData({
          entries: [],
          validationStatus: "Pending",
    ...
    /**
     * Update gigr documents model with new entries
     * @param {Array} entries - gigr document entries
     * @param {string} validationStatus - Validation status
     */
    updateGiGRDocuments: function (entries, validationStatus = "Pending") {
      const oModel = this._view.getModel("gigrDocuments");
      if (oModel) {
