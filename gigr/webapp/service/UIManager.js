sap.ui.define(
  ...
  function (
    ...
    return function (oController, oErrorHandlerInstance) {
      ...
      this.closeDialog = function (dialogId, bDestroy = false) {
        const sFragmentInstanceId = this.oController.getView().createId(dialogId);
        // Prioritize lookup from internal cache
        let oDialog = this._dialogs[dialogId];

        // If not in cache, try core lookup (less likely needed now)
        if (!oDialog || oDialog.bIsDestroyed) {
          oDialog = sap.ui.getCore().byId(sFragmentInstanceId);
        }

        if (oDialog && typeof oDialog.isOpen === 'function') {
          const cleanup = () => {
            if (bDestroy && oDialog && !oDialog.bIsDestroyed) {
              oDialog.destroy();
              delete this._dialogs[dialogId]; // Remove destroyed instance from cache
              console.log(`UIManager: Dialog destroyed: ${dialogId}`);
            }
          };
          if (oDialog.isOpen()) {
            oDialog.close();
            if (bDestroy) { oDialog.attachEventOnce("afterClose", cleanup); }
          } else if (bDestroy && !oDialog.bIsDestroyed) { cleanup(); } // Destroy immediately if already closed
        } else {
          console.warn(`UIManager: Dialog '${dialogId}' not found or invalid for closing.`);
          if (bDestroy) delete this._dialogs[dialogId]; // Clean cache anyway if destroying
        }
      };
      ...
      this.handleValidationErrors = function (errors) {
        ...
        this._loadAndShowDialog(
          ...
          "gigr.view.ValidationErrorsDialog", // <<< ADJUST NAMESPACE
          oValidationModel,
      ...
      this.showEntryDetailsDialog = function (oEntry) {
        ...
        // Note: Date formatting might now need to be done via XML formatters in the fragment
        // or by adding a formatter utility accessible to the main controller if needed before binding.
        // We remove the inline formatDate function here.
        // Example (if needed): oEntryData.FormattedDocumentDate = Formatter.formatDate(oEntryData.DocumentDate);
        // --- End Data Preparation ---

        const dialogId = "entryDetailsDialog";
        const fragmentName = "gigr.view.EntryDetailsDialog"; // <<< ADJUST NAMESPACE
      ...
      this.showErrorSummaryDialog = function (errors, exportManager) {
        ...
        const dialogId = "errorSummaryDialog"; const fragmentName = "gigr.view.ErrorSummaryDialog"; // <<< ADJUST NAMESPACE
      ...
      this.showValidationStatsDialog = function (total = 0, successful = 0, failed = 0) {
        ...
        const dialogId = "validationStatsDialog"; const fragmentName = "gigr.view.ValidationStatsDialog"; // <<< ADJUST NAMESPACE
        // Destroy this dialog on close
      ...
      this.showHelpDialog = function (controllerContext) {
        ...
        const dialogId = "helpDialog"; const fragmentName = "gigr.view.HelpDialog"; // <<< ADJUST NAMESPACE
      ...
      this.showSuccessWithDocuments = function (successEntries) {
        const dialogId = "successDialog"; const fragmentName = "gigr.view.SuccessDialog"; // <<< ADJUST NAMESPACE
      ...
      this.showPartialSuccessWithDocuments = function (successEntries, failedEntries) {
        const dialogId = "partialSuccessDialog"; const fragmentName = "gigr.view.PartialSuccessDialog"; // <<< ADJUST NAMESPACE
      ...
      this.showErrorWithDetails = function (failedEntries) {
        const dialogId = "errorDialog"; const fragmentName = "gigr.view.ErrorDialog"; // <<< ADJUST NAMESPACE
      ...
      this.initBatchProcessingDisplay = function (container) { /* ... */
        ...
        return new Promise((resolve, reject) => {
          ...
          const fragmentName = "gigr.view.BatchProcessingDisplay"; const fragmentId = this.oController.getView().createId("batchProcessingDisplayFragment");
      ...
      this._formatDocumentsForDisplay = function (entries = []) {
        return entries.map((entry) => {
          ...
          let grnDoc = entry.GRNDocumentNumber || "";
          let displayText = grnDoc ? `GRN: ${grnDoc}` : "Document Info Unavailable";
          let formattedDocNumber = grnDoc;
          return {
            sequenceNumber: entry.SequenceNumber || 'N/A',
            grnDocumentNumber: grnDoc,
            material: entry.Material || "",
            plant: entry.Plant || "",
            storageLocation: entry.StorageLocation || "",
            goodsMovementType: entry.GoodsMovementType || "",
            accountAssignment: entry.AccountAssignmentCategory || "",
            wbsElement: entry.WBSElement || "",
            glAccount: entry.GLAccount || "",
            quantity: entry.QuantityInEntryUnit || "",
            entryUnit: entry.EntryUnit || "",
            materialDocumentItemText: entry.MaterialDocumentItemText || "",
            specialStock: entry.SpecialStockIdfgWBSElement || "",
            formattedDocNumber: formattedDocNumber,
            displayText: displayText,
            message: entry.statusMessage || "Success",
      ...
      this._formatErrorsForDisplay = function (entries = []) {
        return entries.map((entry) => {
          ...
          if (entry.ValidationErrors && Array.isArray(entry.ValidationErrors)) {
            entry.ValidationErrors.forEach(valError => {
              const msg = (typeof valError === 'string') ? valError : (valError.message || JSON.stringify(valError));
              if (msg && !errorMessages.includes(msg)) errorMessages.push(msg);
            });
          }

          if (entry.statusMessage && !errorMessages.includes(entry.statusMessage)) {
            errorMessages.push(entry.statusMessage);
          }
          ...
          return {
            ...
            materialDocumentItemText: entry.MaterialDocumentItemText || "",
            specialStock: entry.SpecialStockIdfgWBSElement || "",
            grnDocumentNumber: entry.GRNDocumentNumber || "",
            statusCode: entry.statusCode || "Error",
            statusMessage: entry.statusMessage || errorMessages[0],
